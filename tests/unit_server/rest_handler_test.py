...
def test_rest_handler_setup(requests_mock, shared_key):
...
def test_rest_handler_get_current_user(shared_key):
    a = Auth(shared_key)
    rh = RestHandler()
...
def test_openid_cookie_handler_mixin(shared_key):
    a = Auth(shared_key)
...
def test_openid_login_handler_initialize(requests_mock, shared_key):
